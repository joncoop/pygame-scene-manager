...
class PlayScene2(Scene):
        ...
        for event in events:
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LEFT:
                    self.next_scene = self.game.scenes["play1"]
                    self.terminated = True
                if event.key == pygame.K_RIGHT:
                    self.next_scene = self.game.scenes["end"]
                    self.terminated = True
    ...
    def render(self):
        ...
        text = self.font_xl.render("Play Scene 2", 1, pygame.Color('white'))
        rect = text.get_rect()
