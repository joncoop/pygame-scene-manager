...
class SpecialScene(Scene):
    """
    Represents a special gameplay scene. This example shows how to transition back to the previous scene. 
    It could be used as a menu or inventory type scene.
    """
        ...
        for event in events:
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_DOWN:
                    self.next_scene = self.game.previous_scene
    ...
    def render(self):
        ...
        text = self.font_xl.render("Special Scene", 1, pygame.Color('white'))
        rect = text.get_rect()
